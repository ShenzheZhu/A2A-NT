...
Core experiment code lives at the repository root. Use [`main.py`](main.py) as the entrypoint, [`Conversation.py`](Conversation.py) for negotiation flow, [`LanguageModel.py`](LanguageModel.py) for LiteLLM provider integration, and [`MarkAnomaly.py`](MarkAnomaly.py) for post-run anomaly labeling. Example sweep settings live in [`configs/sweep_example.json`](configs/sweep_example.json). Datasets live in [`dataset/`](dataset), static figures in [`asset/`](asset), and maintained utility/analysis CLIs in [`scripts/`](scripts). Treat `results/`, `logs/`, and `artifacts/` as generated output.
...
- `python scripts/summarize_results.py --results-dir results/sweep --output-dir results/sweep_analysis`
  Generates pair, seller, buyer, and budget analysis tables without notebooks.
...
## Testing Guidelines
There is no formal `pytest` suite in this repository. Validate runner changes with `python scripts/run_sweep.py --dry-run` and a targeted dry run against a small product file; for live provider changes, run one product and one budget before starting a sweep. For analysis CLI edits, run the relevant unit tests and a summary smoke test against a small result directory.

## Commit & Pull Request Guidelines
Recent history uses short imperative subjects such as `Update README.md`, but many older commits are too terse. Prefer clear one-line summaries like `Add retry handling for Gemini responses`. PRs should state the experiment or bug being addressed, list any required config or dataset assumptions, and include representative output paths or summary-table snippets for analysis changes. Never commit API keys from `.env` / `Config.py` or bulk-generated results unless the change is explicitly about curated artifacts.
